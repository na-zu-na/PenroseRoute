...
def authenticate_dispatch_user(authorization: Annotated[str | None, Header()] = None) -> Principal:
    ...
    if configured is None:
        raise AuthenticationError("AUTH_NOT_CONFIGURED", "请配置调度 API 身份凭据", 503)
    try:
        ...
        raise AuthenticationError("AUTH_CONFIG_INVALID", "调度身份配置无效", 503) from exc
    if not authorization or not authorization.startswith("Bearer "):
        raise AuthenticationError("UNAUTHORIZED", "需要 Bearer 凭据", 401)
    supplied = authorization[7:]
    for token, identity in entries.items():
        if hmac.compare_digest(supplied.encode(), token.encode()):
            return Principal(identity["subject"], identity["role"])
    raise AuthenticationError("UNAUTHORIZED", "身份凭据无效", 401)
