...
**默认使用球面距离和固定速度 8.33 m/s 估算行程，结果标记 GEOGRAPHIC_ESTIMATE。没有接入道路、堵车、真实导航或实时 GPS。** 路线几何也未生成。正常 PlanningService 支持注入矩阵 provider；恢复默认适配器仍使用内置估算矩阵，接入真实路网时需统一替换这一计算入口。前端联调用的[模拟车辆位置接口](SIMULATED_GPS.md)只生成只读展示坐标，不代表真实 GPS，也不参与调度或异常恢复。
